...
lexicographically sorted tuples as developed
...
from Theorem~\ref{thm:ibid-dynamicswsffp-sound},
...
As with a conventional bidirectional search,
...
This queue is sorted by the key $d_s(u) + w(u,v) + d_t(v)$.
As an optimization,
the queue does not include edges for which the key is infinite,
since that corresponds to a non-existent path.
...
it is instructive to examine how these efforts have been addressed
in the past.
...
it instead incurs the very high cost of computing full pairwise
...
(its constituent vertices and edges),
...
\begin{figure}
   ...
   \caption{Illustration of behavior of bidirectional heuristic search
      ...
      The top row shows the behavior of the algorithm with no
...
\marginnote{We consider only optimal algorithms (with admissible
heuristics); for an extension to inflated search,
see \citep{aine2016truncatedincremental}.}
...
\end{equation}
Note that the first element of the tuple coincides with the
non-incremental potential function transformation from
(\ref{eqn:ibid:potential-transformation}).
Elements of the product space are sorted lexicographically;
...
\marginnote{Note that in order to maintain $w_b > 0$,
...
\begin{figure}
   ...
   \caption{Expected number of vertex expansions required to solve
      ...
      The ``Replans'' series capture an average over the remaining
      nine episodes.
      Note that
...
(edges blocked with independent probability 0.002),
...
%      balancing strategies on a single-pair road network problem.
...
%      At left, a forward-only balancer is used, so that the
