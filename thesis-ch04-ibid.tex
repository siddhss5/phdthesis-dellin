...
by interleaving a search phase with an evaluation phase.
LazySP conducts a sequence of inner queries using an existing
pathfinding algorithm.
An appropriate selection of this inner algorithm motivates this 

Intro stuff.
Connect to the previous chapters (LazySP).
Solving the dynamic single-pair shortest path (SPSP) problem.
AKA two-terminal shortest-path problem.
Pathfinding.
Difficult parts are near the ends; symmetric.

What if you have weak heuristics.

General idea, combine the following.
All three attempt to reduce the subset of the graph that needs to be
explored.

\begin{enumerate}
\item \emph{Bidirectional Search} -- First.
   Motivation: explore smaller subset of graph.
   The most subtle part of the algorthm is getting the termination
   condition right.
\item \emph{Heuristic Search} -- Second.
   Integration with the first.
   Potential function over vertices.
   With a strong heuristic, can drastically reduce subset of graph
   explored.
\item \emph{Incremental Search} -- Recent.
   Integrated with heuristic.
   Reuse structure from previous searches.
   Only explore the parts that changed.
\end{enumerate}

The principal contribution is an algorithm which combines all three.
