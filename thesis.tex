...
Semi-structured (some things stay the same)
...
\paragraph{Roadmaps}
Focus on roadmap methods.
Can be used to initialize optimization methods.
...
In Chapter~\ref{chap:lazysp}, we discuss the Lazy Shortest Path
algorithm.
...
between searches.
Introduce incremental bidirectional search (IBID) in 
...
\chapter{Review of Roadmap Methods for Motion Planning}

Intro: focus on configuration-space planning (not kinodynamic planning).

Discretation of configuration space.
...
\section{Obstacle Sensitivty vs. Obstacle Agnosticism}
...
\chapter{Application: Caching Invariant Geometry}
...
\chapter{Application: Multi-Step Manipulation Tasks}

\chapter{Application: Exploiting Conservative Geometric Approximations}
