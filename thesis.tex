...
\begin{itemize}
\item Semi-structured (some things stay the same).
...
\include{thesis-ch02-lazysp}

\include{thesis-ch03-ibid}

\include{thesis-ch04-lemur}
...
\begin{figure}
   ...
   \label{fig:herbarmmultithread-master}
\end{figure}

When addressing multi-step tasks
such as the table clearing task in
Figure~\ref{fig:herbarmmultithread-master},
...
\section{Sources of Configuration Space Families}

\section{Optimistic Checks via the Family Graph}

\chapter{Applications}

\section{Caching Invariant Geometry}

\begin{figure*}
   \centering   
   \includegraphics{build/family-sq-caching/pvxs}
   \caption[]{LEMUR single-query results on several HERB problems.
      Using standard OpenRAVE checking calls
      (\protect\tikz{\protect\draw[thick,red] (0,0) -- (0.15,0.15);}),
      baked calls
      (\protect\tikz{\protect\draw[thick,blue] (0,0) -- (0.15,0.15);}),
      or baked calls with self-checked caching
      (\protect\tikz{\protect\draw[thick,green] (0,0) -- (0.15,0.15);}).
      Note that the baked checks don't use an accurate planning cost
      estimate, so the nice $\lambda_U$ shape isn't yet apparent
      (new experiments currently running).}
\end{figure*}

\section{Multi-Step Manipulation Tasks}

\section{Exploiting Conservative Geometric Approximations}
