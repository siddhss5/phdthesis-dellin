...
that A* \cite{hart1968astar} is optimally efficient.
...
factors.
For such domains,
approaches such as partial expansion \cite{yoshizumi2000peastar}
or iterative deepening \cite{korf1985idastar}
...
using roadmap methods \cite{kavrakietal1996prm}.
...
robotic motion planning \cite{bohlin2000lazyprm},
...
%\ssnote{I wonder if it might be better to talk about optimality first 
%before talking about bounded suboptimality, to make things less confusing:
%\begin{theorem}[Optimality of LazyShortestPath]
%If $w_{\ms{est}}$ underestimates $w$, i.e. $w_{\ms{est}}(e) \leq \, w(e)$, and
%\textsc{LazyShortestPath} (Algorithm~\ref{alg:lazy-outline}) terminates
%with some path $p_{\ms{ret}}$,
%then $\mbox{len}(p_{\ms{ret}}, w) = \ell^*$
%with $\ell^*$ the length of an optimal path.
%\label{thm:lazy-optimality}
%\end{theorem}
%}
...
RRT-Connect \cite{kuffner2000rrtconnect}
...
employed by the Lazy PRM \cite{bohlin2000lazyprm}
...
pre-processing step using Dijkstra's algorithm \cite{dijkstra1959anote}
...
The Fast Downward algorithm \cite{helmert2006fastdownward} is motivated
by expensive heuristic evaluations in planning,
whereas the Lazy Weighted A* (LWA*) algorithm \cite{cohen2014narms}
...
As shown in the table in Figure~\ref{subfig:table-results},
