...
%They're very focused on the pre-computation stuff.
...
\begin{table*}
   ...
   \end{minipage}
   \vspace{0.3cm}
   \caption[][0.3cm]{Constituent subsets in the \texttt{herbbin} example
      problem.}
   \label{table:family:herbbin-subsets}
\end{table*}

We measured the length of the path returned by each algorithm (rad),
as well as the total wall clock planning time (s).
...
\begin{table*}
   ...
   \end{minipage}
   \vspace{0.3cm}
   \caption[][0.3cm]{Constituent subsets in the \texttt{workcell} example
      problem.}
