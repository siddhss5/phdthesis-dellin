...
\marginnote{Also called the \emph{joint configuration space}.}
...
\paragraph{Visualizing Manifolds.}
We visualize examples of these manifolds as red surfaces
in Figure~\ref{fig:family:composite-manifolds}.
Below each depiction of the manifold lines a visualization of the
projection of the each composite obstacle in $\mathcal{C}_{RO}$
onto the robot's configuration space $\mathcal{C}_{R}$.
On each manifold,
we also show a solution path that lies on the manifold,
as well as its projection.
...
Project everything down into the robot's configuration space.
